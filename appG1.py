...
ASSET_DEFINITIONS = {"Stocks": "Ownership shares in companies. They can offer higher returns but are more volatile.",
...
ROUNDS = [
    {"name": "Inflation Shock",
     ...
     "impacts": {"Stocks": -0.07,"Currency Market": -0.02,"Fixed Income": -0.09,"Private Assets": -0.04,"Crypto": -0.06},
    ...
    {"name": "Geopolitical Crisis",
     ...
     "impacts": {"Stocks": -0.06,"Currency Market": 0.03,"Fixed Income": 0.02,"Private Assets": -0.05,"Crypto": -0.08},
    ...
    {"name": "Global Banking Crisis",
     ...
     "impacts": {"Stocks": -0.10,"Currency Market": 0.02,"Fixed Income": 0.06,"Private Assets": -0.07,"Crypto": -0.12},
    ...
    {"name": "Interest Rate Hike",
     ...
     "impacts": {"Stocks": -0.05,"Currency Market": 0.02,"Fixed Income": -0.07,"Private Assets": -0.04,"Crypto": -0.09},
    ...
    {"name": "Tech Bubble Burst",
     ...
     "impacts": {"Stocks": -0.12,"Currency Market": 0.00,"Fixed Income": 0.03,"Private Assets": -0.05,"Crypto": -0.15,},
...
def init_state():
    if "phase" not in st.session_state: 
        st.session_state.phase = "setup" #the game starts on allocation display
    if "round_index" not in st.session_state:
        st.session_state.round_index = 0 #event count from 0 to 2 for the 3 shocks
    if "portfolio" not in st.session_state:
        st.session_state.portfolio = {a: 0 for a in ASSETS} #init at 0 for each asset
    if "events" not in st.session_state:
        st.session_state.events = [] #init the list of economic shocks
    if "history" not in st.session_state:
        st.session_state.history = [] #list to store the portfolio's history for the grqph
    if len(st.session_state.events) > 0 and st.session_state.round_index >= len(st.session_state.events):
        st.session_state.phase = "final" #quand 3 events, then it displays the result screen
init_state()

## STRATEGIC FUNCTIONS 
#####################################################################

#fct to reset the game when "Play again" button pressed
#brings every session variable back to the init status
def reset_game():
    ...
    for a in ASSETS:
        st.session_state[f"alloc_{a}"] = 0.0 #reset first allocation value for each asset
        st.session_state[f"rebal_{a}"] = 0.0 #same for rebalance values

#fct that save portfolio's state at a moment (do a snapshot of the portfolio)
def save_history(step_label):
    snapshot = {"Step": step_label} #crée un dictionnaire, Step will be the plot x axis
    for a in ASSETS:
        snapshot[a] = st.session_state.portfolio[a] #each asset value is added to the snapshot
    st.session_state.history.append(snapshot) #add this snapshot to history list

#fct to return a value in euro
def format_euro(x):
    return f"€{x:,.2f}" #thousand separator, 2 digits avec comma, float type, euro sign in front

## DISPLAY FUNCTIONS 
#####################################################################

#build an array (with pandas) of the shock
def build_shock_dataframe(before_portfolio,                                                                         event):
    rows = []
    for a in ASSETS:
        impact = event["impacts"][a] #retrieves impact of this shock on each asset
        after = before_portfolio[a] * (1 + impact) #compute the value after the shock
        rows.append(
            {"Asset": a,
             "Before": round(before_portfolio[a], 2),"Impact": f"{impact * 100:+.0f}%","After": round(after, 2),
             "Before_num": before_portfolio[a],"After_num": after})
    return pd.DataFrame(rows)

#make the array streamlit kind
def style_shock_table(df):
    display_df = df[["Asset", "Before", "Impact", "After"]].copy() #avoid copy warning pandas error

    #colors according to impact value
    ...
    #colors according to beofre/impact difference
    ...
    #activate the pandas Styler for colors and styles to show
    styled = (
        display_df.style
...
#display allocation pie/donut chart
def plot_allocation_donut(portfolio_dict):
    ...
    if not values:
        return #avoid an empty chart

    #creation, parameters and display of the the chart
    fig, ax = plt.subplots(figsize=(4.5, 3.2))
    ax.pie(values,labels=labels,colors=colors,startangle=180,counterclock=False,
           autopct="%1.1f%%",pctdistance=0.8,labeldistance=1,
...
#display evolution lines portfolio graph
def plot_portfolio_history():
    if len(st.session_state.history) < 2:
        return  #pour ne pas avoir un graph d'evolution avec juste 1 point
    ...
    for asset in ASSETS:
        ax.plot(df_hist["Step"],df_hist[asset],marker="o",markersize=5,linewidth=2.2,label=asset,color=ASSET_COLORS[asset])
    ax.plot(df_hist["Step"],df_hist["Total Portfolio"],marker="o",markersize=5,linewidth=2.8,label="Total Portfolio",color="black")
    ...
    for i, asset in enumerate(ASSETS):
        with cols[i]:
            st.number_input(f"{asset} (%)",min_value=0.0,max_value=100.0,
                value=st.session_state.get(f"alloc_{asset}", 0.0),step=1.0,key=f"alloc_{asset}")
...
#####################################################################
    ...
    simulated_portfolio = {a: before[a] * (1 + event["impacts"][a]) for a in ASSETS}
    ...
    with table_col:
        ...
        st.dataframe(style_shock_table(df_shock),use_container_width=True,hide_index=True)
    with pie_col:
...
#####################################################################
    ...
    for i, asset in enumerate(ASSETS):
        with cols[i]:
            ...
            st.number_input(asset,min_value=0.0,max_value=100.0,value=float(round(current_weight, 1)),
                step=1.0,key=f"rebal_{asset}")
...
#####################################################################
    ...
    df_final = pd.DataFrame(
        {"Asset": ASSETS,"Value": [round(st.session_state.portfolio[a], 2) for a in ASSETS]})
    st.dataframe(df_final.style.format({"Value": "€{:,.2f}"}),use_container_width=True,hide_index=True)
    st.subheader("Portfolio history")
    plot_portfolio_history()
